...
import { startNewVotingRound, revealVotes, resetVotingRound } from "@/app/actions/rooms"
...
export function ModeratorControls({ roomId, storyId, isRevealed }: ModeratorControlsProps) {
  ...
  return (
    <Card>
      <CardContent className="p-4 flex flex-wrap items-center justify-center gap-2">
        ...
        <Button onClick={handleReveal} disabled={!storyId || isRevealed || isPending}>
          <Eye className="w-4 h-4 mr-2" />
          Revelar Votos
        </Button>
        <Button onClick={handleReset} variant="outline" disabled={!storyId || isPending}>
